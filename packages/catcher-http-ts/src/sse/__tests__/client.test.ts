...
describe('createSSEClient', () => {
  ...
  describe('基础连接和消费', () => {
    ...
    it('C3 close() 停止迭代', async () => {
      ...
      // Iteration should end after close
      const more: string[] = []
      for await (const line of client) {
        more.push(line)
        break
  ...
  describe('自动重连', () => {
    ...
    it('C6 重连携带 Last-Event-ID', async () => {
      ...
      // 验证第二次 fetch 调用包含 Last-Event-ID
    ...
    it('C8 达到 maxRetries 停止', async () => {
      ...
      // Should stop after maxRetries exceeded — iterator ends
      const lines: string[] = []
      for await (const line of client) {
        lines.push(line)
      }
      expect(lines).toEqual([])
      expect(fetchSpy.mock.calls.length).toBeLessThanOrEqual(3) // initial + retries
    })
  ...
  describe('204 停止重连', () => {
    it('C10 204 停止重连', async () => {
      ...
      // 返回 204 (no body needed but mock needs one)
  ...
  describe('熔断器', () => {
    it('C11 circuitBreaker 集成 — 连续失败后停止', async () => {
      ...
      expect(lines).toEqual([])
      // Should have stopped before hitting maxRetries
