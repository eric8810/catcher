/**
 * High-concurrency throughput benchmark — NAPI (Rust) vs vanilla axios.
 *
 * Mirror of throughput.test.ts using the Rust NAPI adapter instead of
 * the TypeScript catcher-http client.
 *
 * Four scenarios:
 ...
 *   D) Mixed IM workload — realistic request distribution
 *
 ...
 *
 * Usage:
 *   THROUGHPUT_REQUESTS=500 THROUGHPUT_CONCURRENCY=50 pnpm vitest run test/benchmark/napi-throughput.test.ts
 */
...
describe('Scenario A — direct throughput (NAPI)', () => {
  ...
  it(`napi — ${THROUGHPUT_REQUESTS} requests, concurrency=${THROUGHPUT_CONCURRENCY}`, async () => {
...
for (const [key, profile] of Object.entries(NETWORK_PROFILES)) {
  ...
  describe(`Scenario B — ${profile.emoji} ${profile.name} (NAPI)`, () => {
    let server: TestServer
    ...
    it(`napi with retry — ${WEAK_REQUESTS} requests, concurrency=${WEAK_CONCURRENCY}`, async () => {
      proxy.setConditions(profile.conditions)
      proxy.disruptAll()
...
describe('Scenario C — connection efficiency (NAPI)', () => {
  let server: TestServer
  let serverUrl: string
  ...
  it(`napi — 200 sequential, 1 at a time (keepAlive)`, async () => {
    const client = createRustHttpClient({
      baseURL: serverUrl,
      keepAlive: true,
...
describe('Scenario D — mixed IM workload (NAPI)', () => {
  let server: TestServer
  let serverUrl: string
