...
```
@eric8810/catcher-web 包
  ├── 复用 @eric8810/catcher-core 类型
...
// 底层是 fetch()，API 与 @eric8810/catcher-http 一致
...
**结论**：Web 平台推荐**纯 TS 方案**。@eric8810/catcher-http 的 axios 替换为 fetch，韧性层（retry/CB/queue）直接复用 p-retry + cockatiel，不引入 Rust/WASM。
...
|------|--------|--------|--------|------|--------|
| **Node.js (native)** | reqwest | catcher-rs | msgpack | ✅ `@eric8810/catcher-napi-http` — Rust via napi-rs, 已编译 | P0 |
| **Node.js (TS)** | axios | p-retry/cockatiel | msgpackr | ✅ `@eric8810/catcher-http` — API 更丰富（拦截器等） | P0 |
...
| **Web** | fetch (TS) | p-retry/cockatiel | msgpackr | ⚠️ `@eric8810/catcher-web` — **唯一缺失的平台** | P1 |
...
| P1 | Web (`@eric8810/catcher-web`) | 唯一缺失的平台，TS + fetch |
