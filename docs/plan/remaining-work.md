...
| 2 | **@eric8810/catcher-web 拦截器落地** | 完整的 interceptor manager（复用 interceptors.ts 逻辑） | ✅ `interceptors.ts` 实现 createInterceptorManager（use/eject/clear + LIFO request/FIFO response chain），`client.ts` 已集成 |
| 3 | **@eric8810/catcher-web WebSocket** | 原生 WebSocket + 重连封装 | ✅ `ws/client.ts` 实现 createWebSocketClient（exponential backoff 重连、多端点、事件监听） |
...
| 序号 | 项目 | 工作内容 | 预估 |
|------|------|---------|------|
| 6 | **@eric8810/catcher-web 增加 E2E 测试** | 用 playwright/puppeteer 跑 browser 侧验证 | 1h |
...
| 9 | **napi index.d.ts 与 TS @eric8810/catcher-core 类型统一** | napi 用 Rust serde 字段名，TS 用 camelCase，考虑加映射层 | 30min |
...
| 11 | @eric8810/catcher-web 发布到 npm | package.json 就绪，需 publish | 10min |
...
## 四、v3 调研跟进（2026-05 调研闭环后的新增工作）

> 详见以下专项计划文档：

| 文档 | 内容 | 优先级 |
|------|------|:------:|
| `v3-code-fixes.md` | 9 个代码缺陷修复（408/429/DNS/TLS/RetryBudget 等） | 🔴 P0-P1 |
| `v3-architecture-changes.md` | 5 项架构演进（Rate-based CB、DNS 多层 fallback、差异化退避等） | 🔴-🟡 |
| `v3-verification-closure.md` | 6 项验证体系补齐（PBT、Harness 统计、blackhole proxy、新场景等） | 🔴-🟢 |

### v3 关键数字

- 12+ 轮调研迭代、10 个独立实验、4 个被推翻的设计假设
- 7 个 P0 缺口 Q0-Q2 全部可追溯，Q4 待 V1-V6 落地闭合
- 代码修复 ~250 行 + 架构变更 ~500 行 + 验证代码 ~1,035 行
...
- ❌ @eric8810/catcher-web DNS 缓存 — 浏览器不支持
- ❌ @eric8810/catcher-web keepAlive — 浏览器自动管理
