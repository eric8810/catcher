...
| @eric8810/core / http / ws (TS) | ✅ 完整实现，含拦截器 |
| @eric8810/napi-http / napi-ws (.node 已编译) | ✅ 基础 API 可用 |
| proxy.ts — 6 种损伤模型 | ✅ |
...
| 2 | **@eric8810/web 拦截器落地** | 完整的 interceptor manager（复用 interceptors.ts 逻辑） | ✅ `interceptors.ts` 实现 createInterceptorManager（use/eject/clear + LIFO request/FIFO response chain），`client.ts` 已集成 |
| 3 | **@eric8810/web WebSocket** | 原生 WebSocket + 重连封装 | ✅ `ws/client.ts` 实现 createWebSocketClient（exponential backoff 重连、多端点、事件监听） |
...
| 序号 | 项目 | 工作内容 | 预估 |
|------|------|---------|------|
| 6 | **@eric8810/web 增加 E2E 测试** | 用 playwright/puppeteer 跑 browser 侧验证 | 1h |
...
| 9 | **napi index.d.ts 与 TS @eric8810/core 类型统一** | napi 用 Rust serde 字段名，TS 用 camelCase，考虑加映射层 | 30min |
...
| 11 | @eric8810/web 发布到 npm | package.json 就绪，需 publish | 10min |
...
- ❌ @eric8810/web DNS 缓存 — 浏览器不支持
- ❌ @eric8810/web keepAlive — 浏览器自动管理
...
| `docs/arch-ts/10-web.md` | @eric8810/web 架构设计 |